...
async function extractUrl(input) {
  ...
  try {
    res = await fetch(url, {
      ...
      headers: { 'user-agent': 'Mozilla/5.0 (compatible; ClassroomBuzzer/1.0)', accept: 'text/html,application/pdf,text/plain,*/*' },
    });
  } catch (err) {
...
function buildPrompt({ text, count, difficulty, language, instructions }) {
  ...
  return `당신은 교사를 돕는 퀴즈 출제 도우미입니다. 아래 자료를 바탕으로 실시간 퀴즈 게임용 객관식 문제 ${count}개를 만드세요.
...
// 모델 출력 → 버저 문제 형식 {text, time_limit, points, options:[{text,is_correct}]}
