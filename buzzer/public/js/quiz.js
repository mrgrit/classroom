...
(async function () {
  ...
  const $themeRow = document.getElementById('quiz-theme-row');

  try {
    const quiz = await api(`/api/quizzes/${quizId}`);
    $title.value = quiz.title;
    $desc.value = quiz.description;
    $themeRow.insertAdjacentHTML('beforeend', themePickerHtml(quiz.theme));
    bindThemePicker($themeRow, () => setDirty(true));
  ...
  async function save() {
    ...
    const body = {
      ...
      theme: $themeRow.querySelector('.theme-picker')?.dataset.selected || '',
