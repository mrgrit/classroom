...
function createGame(quizId, hostUid) {
  ...
  const pin = genPin();
  const id = db
    .prepare('INSERT INTO games (quiz_id, quiz_title, pin, host_id, status, question_count) VALUES (?, ?, ?, ?, ?, ?)')
    .run(quizId, quiz.title, pin, hostUid, 'lobby', quiz.questions.length).lastInsertRowid;
...
// 카훗 방식 점수: 빠를수록 높음 — 만점 × (1 − 소요시간비율/2), 정답일 때만
