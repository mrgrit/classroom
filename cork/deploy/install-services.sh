#!/usr/bin/env bash
# systemd 유저 서비스로 앱(cork)과 터널(cork-tunnel)을 등록합니다. sudo 불필요.
#   설치:   ./deploy/install-services.sh
#   상태:   systemctl --user status cork cork-tunnel
#   로그:   journalctl --user -u cork -f   /   journalctl --user -u cork-tunnel -f
#   앱 재시작(터널 URL 유지): systemctl --user restart cork
...
cat > "$UNIT_DIR/cork.service" << UNIT
[Unit]
Description=코르크 (Express server)
...
cat > "$UNIT_DIR/cork-tunnel.service" << UNIT
[Unit]
Description=코르크 - Cloudflare Quick Tunnel
After=network-online.target cork.service
Wants=cork.service
...
systemctl --user enable --now cork.service cork-tunnel.service
