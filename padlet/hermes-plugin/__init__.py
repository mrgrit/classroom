"""padlet — Classroom Padlet 연동 플러그인 (Hermes Agent)

무엇을 하나
- ``post_llm_call`` 훅: 매 턴(학생 질문 + 최종 답변)을 패들렛 REST API로 보내 학생 컬럼의 게시물에 누적한다.
  모델이 도구를 부르든 말든 무조건 실행되므로 저장 누락이 없다. (전송은 백그라운드 스레드)
- ``/padlet`` 슬래시 명령: 저장 위치(과목·컬럼)/주제/자동 저장 설정, 상태 확인, 메모, 검색.
- MCP 서버(``PADLET_URL/mcp``)는 install.sh가 config.yaml에 등록한다 — 모델이 쓰는 메모 저장/검색/컨텍스트 변경 도구.

설정: ``~/.hermes/.env`` 의 ``PADLET_URL``, ``PADLET_TOKEN`` (패들렛 '내 정보' 페이지의 설치 명령이 기록함)
"""
...
_state: Dict[str, Any] = {
    "force_new": False,     # /padlet 새글 → 다음 턴은 새 게시물로
...
def _cfg():
    url = os.environ.get("PADLET_URL", "").strip().rstrip("/")
    token = os.environ.get("PADLET_TOKEN", "").strip()
    return url, token
...
def _request(method: str, path: str, body: Any = None, timeout: int = TIMEOUT) -> Any:
    ...
    if not url or not token:
        raise PadletError(
            "PADLET_URL / PADLET_TOKEN이 설정되지 않았습니다. "
            "패들렛 '내 정보' 페이지의 설치 명령을 다시 실행하세요."
        )
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "hermes-padlet-plugin/1.0",
        },
    )
    try:
        ...
        try:
            ...
            pass
        err = PadletError(payload.get("error") or f"HTTP {e.code}")
        err.code = payload.get("code")
        err.status = e.code
        raise err
    except urllib.error.URLError as e:
        raise PadletError(f"패들렛 서버({url})에 연결할 수 없습니다: {e.reason}")
    except (TimeoutError, OSError) as e:
        raise PadletError(f"패들렛 서버 응답 없음: {e}")
...
def _send_turn(payload: Dict[str, Any]) -> None:
    try:
        ...
        logger.warning("padlet 자동 저장 실패: %s", e)
    except Exception as e:  # noqa: BLE001
        with _lock:
            _state["last_error"] = str(e)
        logger.warning("padlet 자동 저장 오류: %s", e)
...
def _on_post_llm_call(
    ...
    **_: Any,
    ...
    t = threading.Thread(target=_send_turn, args=(payload,), name="padlet-save", daemon=True)
    with _lock:
...
USAGE = """/padlet 사용법
  /padlet                 현재 저장 위치·주제·자동 저장 상태
  /padlet 과목 <이름>      과목(보드) 선택 — 이름 일부로 검색 (예: /padlet 과목 네트워크)
  /padlet 컬럼 <이름>      저장할 컬럼 선택
  /padlet 주제 <문구>      주제(단원/예제) 설정 — 그냥 "/padlet 3단원 예제2" 도 됨. 지우기: /padlet 주제 없음
  /padlet 새글             다음 대화부터 새 게시물로 저장
  /padlet on | off         대화 자동 저장 켜기 / 끄기
  /padlet 목록             저장 가능한 과목/컬럼 목록
  /padlet 메모 <내용>      메모 게시물로 바로 저장
  /padlet 검색 <검색어>    내 기록 검색"""
...
def _status() -> str:
    ...
    if not me.get("column"):
        lines.append("ℹ️ 저장 위치가 없습니다. /padlet 과목 <이름> 으로 과목을 선택하세요.")
    return "\n".join(lines)
...
def _cmd(raw_args: str) -> str:
    ...
    try:
        ...
        if key in ("과목", "board", "보드"):
            return _set({"board": rest}) if rest else "과목 이름을 입력하세요. 예: /padlet 과목 네트워크\n" + _targets()
        if key in ("컬럼", "column"):
            return _set({"column": rest}) if rest else "컬럼 이름을 입력하세요. 예: /padlet 컬럼 홍길동\n" + _targets()
        if key in ("주제", "topic"):
            return _set({"topic": rest}) if rest else "주제를 입력하세요. 예: /padlet 주제 3단원 예제2 (지우기: /padlet 주제 없음)"
        if key in ("새글", "new"):
            with _lock:
        ...
        if key in ("메모", "note"):
            if not rest:
                return "메모 내용을 입력하세요. 예: /padlet 메모 TCP 3-way handshake 정리 완료"
        ...
        if key in ("검색", "search", "find"):
            return _search(rest) if rest else "검색어를 입력하세요. 예: /padlet 검색 handshake"
        # 그 외 텍스트는 주제로
        return _set({"topic": args})
    except PadletError as e:
        return f"❌ {e}"
    except Exception as e:  # noqa: BLE001
        logger.exception("/padlet 명령 오류")
        return f"❌ 오류: {e}"
...
def register(ctx) -> None:
    ...
    ctx.register_command(
        "padlet",
        _cmd,
        description="패들렛 연동: 저장 위치(과목/컬럼)·주제·자동 저장 설정, 메모, 검색",
        args_hint="[과목|컬럼|주제|on|off|새글|목록|메모|검색] ...",
    )
    skill = Path(__file__).resolve().parent / "skills" / "padlet" / "SKILL.md"
    if skill.exists():
        try:
            ctx.register_skill("padlet", skill, description="Classroom Padlet 연동 도구 사용법")
        except Exception as e:  # noqa: BLE001
            logger.debug("padlet 스킬 등록 생략: %s", e)
