...
function issueToken(userId) {
  const token = 'crk_' + crypto.randomBytes(24).toString('hex');
  db.prepare(
...
function ensureContext(user) {
  ...
  const hint = ctx.board
    ? `"${ctx.board.title}" 보드에서 저장할 컬럼을 선택하세요 (/cork 컬럼 <이름>). 선택 가능: ${(ctx.targets.find((b) => b.id === ctx.board.id) || { columns: [] }).columns.map((c) => c.title).join(', ')}`
    : `저장할 과목(보드)을 선택하세요 (/cork 과목 <이름>). 선택 가능: ${ctx.targets.map((b) => b.title).join(', ') || '(글을 쓸 수 있는 보드가 없습니다)'}`;
...
function saveTurn(user, input = {}) {
  ...
  if (!ctx.auto_save) return { saved: false, reason: '자동 저장이 꺼져 있습니다. (/cork on 으로 켜기)' };
