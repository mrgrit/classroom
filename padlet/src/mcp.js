...
function buildServer(user) {
  const server = new McpServer({ name: 'classroom-padlet', version: '1.0.0' });

  server.registerTool(
    'padlet_status',
    {
      title: '패들렛 연동 상태',
      description: '패들렛(Classroom Padlet) 연동 상태를 보여준다: 현재 저장 위치(과목 보드·컬럼), 주제(단원/예제), 자동 저장 여부. 학생이 "지금 어디에 저장되고 있어?"라고 물으면 사용.',
      inputSchema: {},
    },
    async () => {
      try {
  ...
  server.registerTool(
    'padlet_list_targets',
    {
      ...
      description: '이 학생이 글을 쓸 수 있는 패들렛 보드(과목)와 컬럼 목록. ★ 표시는 학생에게 지정된 컬럼(자동 저장 기본 위치).',
      inputSchema: {},
    },
    async () => {
  ...
  server.registerTool(
    'padlet_save_note',
    {
      title: '메모/발견 사항 저장',
      description:
        '학생이 "이거 패들렛에 메모해줘", "지금 정리한 내용 저장해줘", "발견한 내용 기록해줘"라고 하면 사용. 현재 컨텍스트의 컬럼에 별도 게시물(메모)로 저장한다. ' +
  ...
  server.registerTool(
    'padlet_search_notes',
    {
      title: '내 기록 검색',
      description:
        '패들렛에 저장된 내 게시물(이전 대화 기록·메모)을 검색한다. 학생이 "지난번에 ~에 대해 뭐라고 했지?", "내가 전에 정리한 ~ 찾아줘"라고 하면 사용. 결과의 발췌를 답변에 활용한다.',
      inputSchema: {
