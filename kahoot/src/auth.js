...
// WebSocket 업그레이드 요청의 쿠키에서 세션 사용자 추출 (없거나 만료되면 null)
function userFromCookieHeader(cookieHeader) {
  const match = /(?:^|;\s*)session=([^;]+)/.exec(cookieHeader || '');
  if (!match) return null;
  try {
    return jwt.verify(decodeURIComponent(match[1]), JWT_SECRET);
  } catch {
    return null;
  }
}
module.exports.userFromCookieHeader = userFromCookieHeader;
