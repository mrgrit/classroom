...
app.get('/api/quizzes', requireAdmin, (req, res) => {
  const quizzes = db
    .prepare(
      `SELECT q.id, q.title, q.description, q.theme, q.created_at, q.updated_at,
...
app.post('/api/quizzes', requireAdmin, (req, res) => {
  ...
  // 배경 테마: 지정하지 않으면 기존 퀴즈들이 가장 적게 쓴 테마를 골라 서로 구별되게 함
  const theme = req.body.theme === undefined
    ? themes.suggestTheme(db.prepare('SELECT theme FROM quizzes').all().map((q) => q.theme))
    : String(req.body.theme || '');
  if (!themes.isValidTheme(theme)) return res.status(400).json({ error: '배경 테마가 올바르지 않습니다.' });
  const id = db
    .prepare('INSERT INTO quizzes (title, description, theme, created_by) VALUES (?, ?, ?, ?)')
    .run(title, (req.body.description || '').trim().slice(0, 300), theme, req.user.uid).lastInsertRowid;
  res.status(201).json({ id, theme });
});
...
app.put('/api/quizzes/:id', requireAdmin, (req, res) => {
  const quiz = db.prepare('SELECT id, theme FROM quizzes WHERE id = ?').get(req.params.id);
  if (!quiz) return res.status(404).json({ error: '퀴즈가 없습니다.' });
  const title = (req.body.title || '').trim().slice(0, 100);
  if (!title) return res.status(400).json({ error: '퀴즈 제목을 입력하세요.' });
  const theme = req.body.theme === undefined ? quiz.theme : String(req.body.theme || '');
  if (!themes.isValidTheme(theme)) return res.status(400).json({ error: '배경 테마가 올바르지 않습니다.' });
  let questions;
  try {
  ...
  db.transaction(() => {
    db.prepare("UPDATE quizzes SET title = ?, description = ?, theme = ?, updated_at = datetime('now') WHERE id = ?").run(
      title, (req.body.description || '').trim().slice(0, 300), theme, quiz.id
    );
...
app.post('/api/quizzes/:id/duplicate', requireAdmin, (req, res) => {
  ...
  const id = db.transaction(() => {
    const newId = db
      .prepare('INSERT INTO quizzes (title, description, theme, created_by) VALUES (?, ?, ?, ?)')
      .run(`${quiz.title} (복사본)`.slice(0, 100), quiz.description, quiz.theme || '', req.user.uid).lastInsertRowid;
    const insQ = db.prepare('INSERT INTO questions (quiz_id, position, text, time_limit, points) VALUES (?, ?, ?, ?, ?)');
    const insO = db.prepare('INSERT INTO options (question_id, position, text, is_correct) VALUES (?, ?, ?, ?)');
...
app.get('/api/games/:id', requireAuth, (req, res) => {
  ...
  if (live) return res.json({ id: live.id, pin: live.pin, title: live.title, status: live.status, theme: live.theme, live: true });
  const row = db.prepare('SELECT id, pin, quiz_title AS title, status, theme FROM games WHERE id = ?').get(req.params.id);
...
app.get('/api/games/:id/results.csv', requireAdmin, (req, res) => {
  ...
  res.setHeader('Content-Disposition', `attachment; filename="buzzer-${data.game.id}.csv"`);
  res.send('\uFEFF' + lines.join('\r\n')); // BOM: 엑셀 한글 깨짐 방지
});
...
server.listen(PORT, () => {
  console.log(`버저(Buzzer) 서버 실행 중: http://localhost:${PORT}`);
