...
| 디렉토리 | 도구 | 설명 | 상태 |
|---|---|---|---|
| [`cork/`](./cork) | **코르크 (Cork)** | 코르크 게시판 — 담벼락(보드)에 학생들이 게시물을 올리고 좋아요/댓글. 보드별 배경 테마, 보드 접근권한, 글쓰기 양식(관리자가 `/admin`에서 제작, 예: 주간업무보고), markdown 내보내기, 학생별 Ollama로 컬럼 기록 → 개인 학습자료 PDF 생성, AI 에이전트 대화 자동 저장 연동 — 헤르메스(Hermes Agent) 플러그인 + Claude Code 훅, 공용 MCP 서버 | ✅ 사용 가능 |
